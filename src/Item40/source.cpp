...
class DatabaseID : public IPerson {
 public:
  ...
  virtual std::string birth_date() const { return birth_date_; }

  void set_name(const std::string& str) { name_ = std::move(const_cast<std::string&>(str)); }
...
const char* PersonInfo::the_name() const {
  ...
  std::snprintf(value, sizeof(value), "%s%s%s", ValueDelimOpen(), pid_.name().c_str(),
                ValueDelimClose());
  return value;
}
...
const char* PersonInfo::the_birth_date() const {
  static char value[kMaxFormattedFieldValueLength];
  std::snprintf(value, sizeof(value), "%s%s%s", ValueDelimOpen(), pid_.birth_date().c_str(),
                ValueDelimClose());
  return value;
}
