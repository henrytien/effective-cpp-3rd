...
//
// template<typename T>
// NamedObject<T>::NamedObject(const std::string& name, const T& value) : nameValue(name),
// objectValue(value) { 	std::cout << "const string: " << name << " " << value << std::endl;
//}
...
template <typename T>
NamedObject<T>::NamedObject(const NamedObject& obj)
