...
class Font {  // RAII class
 public:
  ...
  void releaseFont(FontHandle f) { f.~FontHandle(); }
  operator FontHandle() const { return f; }  // implicitly conversion function
  FontHandle get() const { return f; }       // explicit conversion function
  void changeFontSize(FontHandle f, int newSize) { f.SetFontSize(newSize); }
...
int main() {
  std::cout << "Item 15: Provide access to raw resources in resource managing classes."
            << std::endl;
  {
    Font f(getFont());
    int newFontSize = 10;
