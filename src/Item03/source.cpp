...
class CTextBlock {
 public:
  ...
  void print(const CTextBlock& cctb) { std::cout << cctb.pText << std::endl; }
  void test();

 private:
  void log(const char* msg) { std::cout << "[" << this << "]" << msg << std::endl; }
  char* pText;
  mutable std::size_t textLength;
  mutable bool lengthValid;  //
};

CTextBlock::CTextBlock(std::string const& str)
    : pText(const_cast<char*>(str.c_str())), textLength(0), lengthValid(false) {}
